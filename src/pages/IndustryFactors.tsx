/**
 * 功能名称：行业系数管理
 * 实现原理与逻辑：维护按行业（政府、教育、商业等）的调整系数，支持乘数/加数及目标字段配置；按行业、系统、启用状态筛选，
 * 新增/编辑/删除。用于报价或成本按行业差异化计算。数据来自 /api/industry-factors。
 */
...
const IndustryFactorsPage: React.FC = () => {
  ...
  const fetchList = useCallback(async () => {
    ...
    try {
      ...
      if (industryFilter) params.set('industry_code', industryFilter)
      if (systemFilter) params.set('system_type', systemFilter)
      if (enabledFilter !== undefined) params.set('enabled', enabledFilter)
      const res = await axios.get<{ list: Row[]; total: number }>(`/api/industry-factors?${params.toString()}`)
  ...
  }, [industryFilter, systemFilter, enabledFilter, msg])
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (editingId) {
        await axios.put(`/api/industry-factors/${editingId}`, payload)
        msg.success('更新成功')
      } else {
        await axios.post('/api/industry-factors', payload)
        msg.success('创建成功')
      }
      setModalOpen(false)
      fetchList()
  ...
  const handleDelete = async (id: number) => {
    try {
      await axios.delete(`/api/industry-factors/${id}`)
      msg.success('已删除')
      fetchList()
    } catch (e: unknown) {
  ...
  const columns: ColumnsType<Row> = [
    ...
    { title: '行业', dataIndex: 'industry_code', width: 100 },
  ...
  return (
    <Space direction="vertical" size={16} style={{ width: '100%' }}>
      <Title level={4} style={{ marginBottom: 0 }}>行业修正系数</Title>
      <Text type="secondary">按行业（政府/教育/商业/工业等）对数量或单价做乘数/加数修正。</Text>
      <Card>
        <Space wrap style={{ marginBottom: 16 }}>
          <Select placeholder="行业" allowClear style={{ width: 120 }} value={industryFilter} onChange={setIndustryFilter} options={INDUSTRY_CODES} />
      ...
      <Modal title={editingId ? '编辑行业修正系数' : '新增行业修正系数'} open={modalOpen} onOk={handleSubmit} onCancel={() => setModalOpen(false)} width={520} destroyOnHidden>
        <Form form={form} layout="vertical">
          ...
          <Form.Item name="industry_code" label="行业编码" rules={[{ required: true }]}><Select options={INDUSTRY_CODES} placeholder="选择行业" /></Form.Item>
...
export default IndustryFactorsPage
